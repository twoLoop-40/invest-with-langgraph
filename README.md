...
1. `notebooks` 폴더 클릭
2. 순서대로 실습:
   - **1단계**: `1 generate.ipynb` (기본)
   - **2단계**: `2 web_search.ipynb` (웹 검색)
   - **3단계**: `3_tool_agent.ipynb` (도구 + 10개 실습)
...
**추천**: LangGraph를 처음 접하는 사람 | **소요시간**: 15-20분
...
**워크플로우:**
```
질문 입력 → AI 답변 생성 → 품질 평가 → 15점 미만이면 웹 검색 → 재생성 → 완료
```

**추천**: 1번 노트북 완료한 사람 | **소요시간**: 30-40분

---

### 3️⃣ `3_tool_agent.ipynb` - Tool 기반 ReAct Agent

**배우는 내용:**
- LangChain Tool 프레임워크 사용법
- ReAct 패턴 (Reasoning + Acting)
- 4개의 투자 분석 도구 활용
- 10개의 실습 문제로 단계별 학습

**사용 가능한 도구:**
1. `search_web` - 웹에서 최신 뉴스 검색
2. `get_stock_price` - 주식 가격 조회
3. `calculate_moving_average` - 이동평균선 계산
4. `get_company_info` - 기업 정보 조회

**실습 문제:**
- 문제 1-3: 도구 사용법 익히기
- 문제 4-6: yfinance로 주가 데이터 다루기
- 문제 7-8: 나만의 도구 만들기
- 문제 9-10: Agent 상태 관리 및 창의적 질문 만들기

**추천**: 2번 노트북 완료한 사람 | **소요시간**: 60-90분
